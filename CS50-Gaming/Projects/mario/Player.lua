...
function Player:init(map)
    ...
    self.behaviors = {
        ...
        ['end'] = function(dt)
            love.graphics.printf('The game is over! Press space to restart', 0, 10, VIRTUAL_WIDTH, 'center')
...
function Player:update(dt)
    -- get going undeground working
    if (self.x >= map.tileHeight * map.mapWidth) or ((self.state ~= 'end') and (self.y > (map.tileHeight * ((map.mapHeight - 2) / 2) - self.height + (2 * map.tileHeight))))  then
        self.state = 'end'
    end
